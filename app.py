...
def ensure_model_artifacts():
    required_files = [
        ...
        'model/binary_mode.pkl',
        'model/quality_threshold.pkl',
    ]
    missing = [f for f in required_files if not os.path.exists(f)]
    if not missing:
        return

    st.warning('⚠️ Model artifacts not found; training model now (this may take 2-5 minutes).')
    app_dir = pathlib.Path(__file__).resolve().parent
    train_script = app_dir / 'train.py'
    if not train_script.exists():
        st.error('❌ train.py not found in project directory. Cannot auto-train.')
        st.stop()

    try:
        # Run training script to generate artifacts
        ...
        st.error('❌ Auto-training failed. Review logs below:')
        st.code(exc.stderr or exc.stdout)
        st.stop()
    except Exception as exc:
        st.error('❌ Unexpected error during auto-training.')
...
def load_model_artifacts():
    ...
    try:
        ...
        st.error('⚠️ Model not found after auto-training. Please run `python train.py` manually.')
        st.stop()
